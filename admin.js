  ...
  async function main(){
    ...
    els.form?.addEventListener('submit', async (ev) => {
      ...
      try {
        ...
        if (!validProduct(payload)){
          els.formMsg.textContent = 'Please fill all fields correctly (Name, Price, Image URL, Category).';
          els.formMsg.className = 'text-sm text-red-700';
          return;
        }
        await db.collection('products').add(payload);
        els.formMsg.textContent = 'Product saved! It will appear on the site shortly.';
        els.formMsg.className = 'text-sm text-green-700';
        els.form?.reset();
        refreshList();
      } catch (e){
        els.formMsg.textContent = (e?.message || String(e));
        els.formMsg.className = 'text-sm text-red-700';
      }
    });
